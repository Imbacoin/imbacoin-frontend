import React, { useEffect, useRef, useState} from "react";

import GoButton from "./GoButton";
import BuyForm from "./BuyForm";
import vorota from "../images/mobile/compressed/vorota.png"
import vorotaopen from "../images/mobile/compressed/vorotaopen.png"
import vorota_uzor from "../images/mobile/compressed/vorota_uzor.png"
import lines from "../images/mobile/compressed/lines.png"
import path_left_lite from "../images/mobile/compressed/path_left_lite.png"
import path_right_bottom from "../images/mobile/compressed/path_right_bottom.png"
import player from "../images/mobile/compressed/player.png"
import right_top_corner from "../images/mobile/compressed/right_top_corner.png"
import tablo from "../images/mobile/compressed/tablo.png"
import tablo_down_brizg from "../images/mobile/compressed/tablo_down_brizg.png"
import tablo_right_top_brizg from "../images/mobile/compressed/tablo_right_top_brizg.png"
import tablo_bg_lines from "../images/mobile/compressed/tablo_bg_lines.png"
import tablo_lenti from "../images/mobile/compressed/tablo_lenti.png"
import zritel from "../images/mobile/compressed/zritel.png"
import bg from "../images/mobile/compressed/bg.png"
import ball from "../images/mobile/compressed/ball.png"
import ball_oreol from "../images/mobile/compressed/ball_oreol.png"
import left_light from "../images/mobile/compressed/left_light.png"
import france from "../images/lang/france.png";
import arrow_next from "../images/lang/arrowDown.svg";
import arrow_mob from "../images/lang/arrow_mob.svg";
import path_all from "../images/mobile/compressed/path_all.png";
import path_mob from "../images/mobile/compressed/path.svg";
import SearchBar from "./searchBar";
import {gsap} from "gsap";
import  '../App_mobile.scss'
import BeatLoader from "react-spinners/BeatLoader";

function IntroMobile() {
    gsap.ticker.lagSmoothing(500, 33)

    const langFullPanelRef = useRef(null)
    const langRef = useRef(null)
    const tl_start = useRef(null)
    const tl_intro = useRef(null)
    const tl_Ref = useRef(null)
    const tl_langPanel_Ref = useRef(null)
    const [start, setStart] = useState(false)
    const containerRef = useRef(null)
  //  gsap.config({ nullTargetWarn: false });
      

    const q = gsap.utils.selector(containerRef)
  
    const cnElems = document.getElementsByClassName("root").innerHeight;
    const wHeight = window.innerHeight;
      

    const [isLoading, setIsLoading] = useState(true)
    const imgs = [
        path_all,
        player,
        ball,
        bg,
        ball_oreol,
        left_light,
        tablo,
        vorota,
        tablo_down_brizg,
        vorota_uzor,
        zritel,
         
    ]

    useEffect(() => {
        const loadImage = src => {
            return new Promise((resolve, reject) => {
                const loadImg = new Image()
                loadImg.src = src
                loadImg.onload = () => {
                        resolve(src)
                }
                loadImg.onerror = err => reject(err)
            })
        }

        Promise.all(imgs.map(src => loadImage(src)))
            .then(() => {
                setIsLoading(false)
            })
            .catch(err => console.log("Failed to load images", err))
    }, [])

    useEffect(() => {

        if (!isLoading) {


        const circles = q('.circle')
        const squares = q('.squares')

        const minX = 0;
        const maxX = 200

        const minY = window.innerHeight;
        const maxY = window.innerHeight / 2;

        const minSize = window.innerHeight / 100;
        const maxSize = window.innerHeight * 4 / 100;

        const minDelay = 0;
        const maxDelay = 3;

        const minOpacity = 1.;
        const maxOpacity = 1.0;

        const minDuration = 1;
        const maxDuration = 3;

        const minRotation = 15;
        const maxRotation = 60;

     

   //    setTimeout(() => {
        
       
       circles.forEach(el => {
            animateCircle(el, 0);
        })

        squares.forEach(el => {
            animateCircle(el, 1);
        }) 
        

        function animateCircle(el, sq) {

            const x = random(minX, maxX);
            const y = random(minY, maxY);
            let sizeH, sizeW
            if (sq) {
                sizeW = 2 * random(minSize, maxSize);
                sizeH = random(minSize, maxSize);
            } else {
                sizeW = random(minSize, maxSize);
                sizeH = random(minSize, maxSize);
            }
            const delay = random(minDelay, maxDelay);
            const rotation = random(minRotation, maxRotation);
            const opacity = random(minOpacity, maxOpacity);
            const duration = random(minDuration, maxDuration);

            gsap.set(el, {
                x: x,
                y: sizeW,
                rotation: rotation,
                width: sizeW,
                height: sizeH,
                opacity: opacity
            });

            gsap.to(el, {
                duration,
                opacity: 0,
                rotation: rotation,
                y: y,
                x: x,
                delay: delay,
                onComplete: animateCircle,
                onCompleteParams: [el]
            });
        }

        function random(min, max) {
            if (max == null) {
                max = min;
                min = 0;
            }
            return Math.random() * (max - min) + min;
        }
        

      gsap.to(".mobile_left_light1", {
            rotation: -10,
            transformOrigin: '0 0',
            duration: 15,
            repeat: -1,
            yoyo: true,
            ease: "none"
        })
        gsap.to(".mobile_left_light2", {
            rotation: 20,
            transformOrigin: '0 0',
            duration: 17,
            repeat: -1,
            yoyo: true,
            ease: "none"
        })
        gsap.to(".mobile_left_light3", {
            rotation: 10,
            transformOrigin: '0 0',
            duration: 13,
            repeat: -1,
            yoyo: true,
            ease: "none"
        })
 
      
        let ctx = gsap.context(() => {
          
          const anim_path =  gsap.timeline({repeat: -1, repeatDelay:1.3, paused:true}).fromTo("#circleMask", 0.5, {scaleY:0}, {scaleY:1}, '<+=0.5' );
          


          //fromTo("#circleMask", {y:"100%"}, {y:"0%",  yoyo: true},  "+=0.5");
          anim_path.pause()
           tl_intro.current = gsap.timeline({paused: true})
                 .set('.mobile_intro_wrap', { height: wHeight})
                 .set('.buy_wrap', { height: wHeight})
           /*      //  .to(q(".scene"), {scale: 2, xPercent: -50, yPercent: 20, duration: 1, ease: "back"}) */
                .fromTo(q(".mobile_bg"), { yPercent: 200},{ yPercent: 0, duration: 1, ease: "back"},"<+0.2")
                .fromTo(q(".mobile_right_top_corner"), {scale: 0, yPercent: -20, xPercent:20}, 
                    {scale: 1,  yPercent: 0, xPercent:0, duration: 1, ease: "power3.inOut"}, "<+=0.2")
                .to(q(".mobile_lines"), {opacity: 1, x: 0, duration: 1, ease: "power3.inOut"}, "<+=0.2") 
                .fromTo(q(".mobile_vorota,.mobile_vorotaopen"), { xPercent: 100},{ xPercent: 0, duration: 1, ease: "back"}, "<+=0.2")
              //  .to(q(".scene"), {scale: 1, xPercent: 0, yPercent: 0, duration: 1, ease: "power3.inOut"})  
                 .fromTo(q(".mobile_tablo"), {yPercent: -250, xPercent: -80},{yPercent: 0, xPercent: 0, duration: 1, ease: "back"},"<+=0.2")
                .fromTo(q(".mobile_tablo_lenti"), {yPercent: -250, xPercent: -80},{ yPercent: 0, xPercent: 0, duration: 1, ease: "none"}, "<+=0.2")
                .fromTo(q(".mobile_tablo_right_top_brizg"),
                    {opacity: 0, xPercent: -8},
                    {opacity: 1, xPercent: 0, duration: 0.5, ease: "back"}, "<+=0.2")
                .fromTo(q(".mobile_tablo_down_brizg"),
                    {opacity: 0, yPercent: -100},
                    {opacity: 1, yPercent: 0, duration: 0.5, ease: "back"}, "<+=0.2")
                .fromTo(q(".mobile_tablo_bg_lines"),
                    {opacity: 0, yPercent: -30},
                    {yPercent: 0, opacity: 1, duration: 1, ease: "power4.inOut"},"<+=0.2")
                .fromTo([".mobile_left_light1",".mobile_left_light2",".mobile_left_light3"],
                   {xPercent: -100, yPercent: -100},
                    {xPercent: 0, yPercent: 0, stagger:0.2, duration: 1}, "<+=0.2")
                .fromTo(q(".mobile_confetti"),{opacity:0},
                   {opacity: 1, duration: 3}, "<+=0.2")   
                .fromTo(q(".mobile_player"),
                    { opacity:0, rotation: 30,yPercent: 10, xPercent: -50},
                    {opacity: 1, rotation: 0, yPercent: 0, xPercent: 0, duration: 1, onStart:()=> {
                      anim_path.play()

                    }, ease: "back"}, "<+0.2")
               //    .fromTo(q(".path"), {opacity: 0},{opacity: 1, duration: 0.3, stagger: 0.1, ease:"back"}, "<+=0.2")
                //  .fromTo(q('.path'), {opacity: 0},  {opacity: 1, duration: 1, stagger: 0.1, repeat:-1, repeatDelay:1},"<") 
                .fromTo(q(".mobile_path_left_lite"),
                    { scale:0, xPercent: -80, yPercent: 200},
                    {scale: 1, xPercent: -35,yPercent: 5, duration: 0.3, ease: "power3.Out"}, "<+=1")
                .fromTo(q(".mobile_path_right_bottom"),
                    { scale:0, xPercent: 50, yPercent: 200},
                    { scale:1, xPercent: 0, yPercent: 0,duration: 0.3, ease: "power3.Out"}, "<+=0.1")   
            .fromTo(q(".mobile_ball"),
                    {scale: 0},
                    {scale: 1, duration: 0.3, ease: "back"}, "<+=0.5")  
                .fromTo(q(".mobile_ball_oreol"),
                    {scale: 0},
                    {scale: 1, duration: 0.2, ease: "back"}, "<+=0.2")   
                .fromTo(q(".mobile_vorotaopen"),
                    {opacity:0},
                    {opacity:1, duration: 0.2}, "<+=0")  
                .fromTo(q(".mobile_vorota"),
                    {opacity:1},
                    {opacity:0, duration: 0.2}, "<+=0.3")  

                    .fromTo(q(".mobile_vorota_uzor"),
                    {scale: 0},
                    {scale: 1, duration: 0.5, ease:"bounce"}, "<+=0.4")     
                .fromTo(q(".mobile_zritel"),
                    { xPercent: 100, yPercent: 100},
                    { xPercent: 0, yPercent: 0, duration: 1, ease: "back"}, "<-=0.5")
                .fromTo(q(".goButton"),
                    {yPercent: 200},
                    {yPercent: 10,duration: 1,ease: "back"}, "<")
                .to('.appears', {opacity: 1, duration: 1})  

 
        });
    
  //  setTimeout(() => {

   
    const suspenses =  q('.suspense')
        
    suspenses.forEach(el => {
        el.style.setProperty('visibility', 'visible');
   })

     
      //  } , 1000)
        tl_intro.current.play()
        return ()=>   ctx.revert()   
        

  
        }
    }, [isLoading])

    const startForm = () => {
        setStart(true)
        tl_start.current = gsap.timeline({paused: true})
            .set(q(".mobile_player"), {zIndex: 10})
            .set(q(".mobile_zritel"), {zIndex: 0})
            .set(".buy_wrap", {top: 0, zIndex: 9})
            .to(".buy_form_wrap", {opacity: 1, duration: 1, ease: "power4.inOut"})
            .to(q(".middle_box"), {top: '0vh', duration: 2, ease: "power4.inOut"}, "<")
            .to(q(".mobile_player_wrap"), {bottom: '1%', scale:1.2, left: -10, duration: 2, ease: "power4.inOut"}, "<")
            .to('.chat_wrap', {top: '70vh', height: '15vh', duration: 2, ease: "power4.inOut"}, "<")
            .to('.goButton', {zIndex:0}, "<")

        tl_start.current.play()
    }

    const back_to_main = () => {
        tl_start.current.reverse()

    }  
    useEffect(() => {
        tl_Ref.current = gsap.timeline({paused: true})
        tl_Ref.current.to(".chat_wrap", {autoAlpha: 0, ease: 'power3.inOut'})
        tl_Ref.current.to(".social_buttons_wrap", {autoAlpha: 0, ease: 'power3.inOut'})
        tl_Ref.current.to(q(".lang_panel"), {autoAlpha: 1, bottom: '0', ease: 'power3.inOut'})
    }, [tl_Ref, q])

    useEffect(() => {
        tl_langPanel_Ref.current = gsap.timeline({paused: true})
        tl_langPanel_Ref.current.to(q(".go_title"), {opacity: 0, y: 50, ease: 'none'})
    }, [tl_langPanel_Ref, q])

    const langClick = () => {
         console.log('langclick')
        if (!langRef.current.classList.contains('active')) {
            langRef.current.classList.add('active')
            tl_Ref.current.play()
        } else {
            langRef.current.classList.remove('active')
            tl_Ref.current.reverse()
        }
    }
    const subLangClick = () => {
        langRef.current.classList.remove('active')
        tl_Ref.current.reverse()
    }

    const openLangPanel = () => {
        if (!langFullPanelRef.current.classList.contains("active")) {
            langFullPanelRef.current.classList.add("active")
            gsap.timeline().to(q(".lang_full_panel"), {autoAlpha: 1, duration: 0.3})
                .to(q(".panel_l"), {marginTop: 0, opacity: 1, duration: 1, ease: "back"}, "<")
                .fromTo(q(".liLang"), {opacity: 0}, {opacity: 1, duration: 1, ease: "back"}, "<")
        } else {
            langFullPanelRef.current.classList.remove("active")
        }
    }

    const closeLangPanel = () => {
        if (langFullPanelRef.current.classList.contains("active")) {
            gsap.timeline().to(q(".panel_l"), {marginTop: 100, duration: 1, ease: "back"})
                .to(q(".lang_full_panel"), {autoAlpha: 0, duration: 1.2}, "<")
            langFullPanelRef.current.classList.remove("active")
            langRef.current.classList.remove('active')
            tl_Ref.current.reverse()
        }
    }

    const wrapClosePanel = (e) => {
        if (e.target.classList.contains("lang_full_wrap")) {
            gsap.timeline().to(q(".panel_l"), {marginTop: 100, duration: 1, ease: "back"})
                .to(q(".lang_full_panel"), {autoAlpha: 0, duration: 1.2}, "<")
            langFullPanelRef.current.classList.remove("active")
        }
    }

    const langs = [
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france},
        {lang: 'fran', img: france}
    ]


    return (
         
         

        <div className="mobile_intro_wrap" id="root"  ref={containerRef}>
                
                {isLoading ? (
                    <div className="preloader">
                        <BeatLoader color="#1d275f"/>
                    </div>
                )
                : (     
                    <div>
                <div className="preloader">
                        <BeatLoader color="#1d275f"/>
                    </div>  
              
                     
          
            <div className="mobile_scene">
             <div className="intro_scene">
                  

                    <div className="mobile_bg mobile_img  suspense"><img alt="" src={bg}/></div>
                    <div className="mobile_lines mobile_img  suspense"><img alt="" src={lines}/></div>
                    <div className="mobile_right_top_corner mobile_img  suspense"><img alt="" src={right_top_corner}/></div>
                     
                    <div className="mobile_confetti">
                        <div className="circle"></div>
                        <div className="circle"></div>
                        <div className="circle"></div>
                        <div className="circle"></div>
                        <div className="circle"></div>
                        <div className="circle"></div>     
                        <div className="squares"></div>
                        <div className="squares"></div>
                        <div className="squares"></div>
                        <div className="squares"></div>
                        <div className="squares"></div>
                        <div className="squares"></div>
                        <div className="squares"></div> 
                    </div>




                    <div className="mobile_tablo_wrap">    </div>
                    <img className="mobile_tablo_bg_lines mobile_img  suspense" alt="" src={tablo_bg_lines}/>
                    <img className="mobile_tablo_lenti mobile_img  suspense" alt="" src={tablo_lenti}/>

                    <img className="mobile_left_light1 mobile_img suspense" alt="" src={left_light}/> 
                    <img className="mobile_left_light2 mobile_img suspense"alt="" src={left_light}/> 
                    <img className="mobile_left_light3 mobile_img suspense" alt="" src={left_light}/> 

                    <img className="mobile_tablo mobile_img  suspense" alt="" src={tablo}/>
                    <img className="mobile_tablo_down_brizg mobile_img  suspense" alt="" src={tablo_down_brizg}/>
                    <img className="mobile_tablo_right_top_brizg mobile_img  suspense" alt="" src={tablo_right_top_brizg}/>
                
                    
                    <div className="mobile_vorota_wrap"></div>
                    <img className="mobile_vorota mobile_img suspense" alt="" src={vorota}/>
                    <img className="mobile_vorotaopen mobile_img suspense" alt="" src={vorotaopen}/>
                    <img className="mobile_vorota_uzor mobile_img suspense" alt="" src={vorota_uzor}/>
                    <img className="mobile_ball_oreol mobile_img suspense" alt="" src={ball_oreol}/>
                    <img className="mobile_ball mobile_img suspense" alt="" src={ball}/>
                   
                   
                    <svg id="Layer_path"  className="suspense" data-name="Layer 1" width="100%" viewBox="0 0 168 331">
                    <defs>
                        <mask id="theMask"><rect id ="circleMask"  className="mask-a" x="1" y="1"   width="100%" height="100%"/></mask>
                    </defs>
                    <image mask="url(#theMask)" width="167" height="330" transform="translate(1)" href={path_all}/>

                    </svg>
                    <img className="mobile_path_left_lite mobile_img suspense" alt="" src={path_left_lite}/>
                    <img className="mobile_path_right_bottom mobile_img suspense" alt="" src={path_right_bottom}/>
                    <div className="mobile_player_wrap">
                     <img className="mobile_player mobile_img  suspense" alt=""  src={player}/>
             </div>  

                  
              
 
            </div>    
           
            <img className="mobile_zritel mobile_img suspense" alt="" src={zritel}/> 
            
            </div>

            <div className="goButton"> 
                      <GoButton startForm={startForm} start={start} langClick={langClick} />  
              </div>  
            
                   </div>  
                )}
           

            <div className="lang_panel" ref={langRef}>
                    <ul>
                        {
                            langs.slice(0, 5).map((el, index) => (
                                    <li key={index} onClick={subLangClick} className="liLang">
                                        <div className="name">{el.lang}</div>
                                        <div className="flag suspense">
                                            <img alt="" src={el.img}/>
                                        </div>
                                    </li>
                                )
                            )
                        }
                    </ul>
                    <div className="arrow_mob suspense" onClick={openLangPanel}>
                        <img alt="" src={arrow_mob}/>
                    </div>
                    <div className="next_arrow suspense" onClick={openLangPanel}>
                        <img alt="" src={arrow_next}/>
                    </div>
                </div>

                <div className="lang_full_panel" ref={langFullPanelRef}>
                    <div className="lang_full_wrap" onClick={wrapClosePanel}>
                        <div className="panel_l">
                            <div className="panel_close_btn" onClick={closeLangPanel}>
                                <span></span>
                                <span></span>
                            </div>
                            <div className="search_pos">
                                <SearchBar/>
                            </div>
                            <div className="allLangs">
                                <ul>
                                    {
                                        langs.map((el, index) => (
                                                <li key={index} className="liLang">
                                                    <div className="name">{el.lang}</div>
                                                    <div className="flag">
                                                        <img alt="" src={el.img} />
                                                    </div>
                                                </li>
                                            )
                                        )
                                    }
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="buy_wrap">
                    <BuyForm back_to_main={back_to_main}/>
                </div> 
                  
            
        </div>
        
    );
}

export default IntroMobile;